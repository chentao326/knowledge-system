---
id: 2026-04-10-samuel-compound-knowledge-system-001
title: "用 AI 搭一套会复利的知识系统"
source_type: pdf
...
这篇文档大部分内容，是 AI 帮我写的。你不需要每个字去阅读，只要了解制作流程和思想，然后发给 AI，让 AI 开发每个技能。或者不用技能，只让 AI 写好提示词。然后测试，根据不满意的地方，让 AI 调整提示词即可。

当然你实在很懒也可以复制文档里的提示词。但每个人使用场景不同，不存在适用于所有人的提示词。比如你想摄取小红书文章，另一个人摄取公众号。都会遇到不同的问题。但都可以让 AI 自己研究一个方案，成功后，让 AI 更新到提示词里。随着使用，提示词越来越强大，成为你个人的资产。

### 其他相关懒人包

- Claude Code 电子书 & AI 全自动开发系统实战【数字游牧人懒人包】
- AI + PPT 制作技巧指南【数字游牧人懒人包】
- Anthropic 长程应用开发中的驾驭框架设计【数字游牧人懒人包】

### 引言

AI 大神 Andrej Karpathy 最近发布了关于用 AI 做知识管理、做个人 wiki 的文章，阅读量到了 1800 万。
...
备注：本篇内容中的配图，也是用文档中提到的方法论制作。生成这种配图，可以使用李继刚老师的 skills。
...
5. 输出必须符合给定 schema。
6. 不要改写事实，不要补充外部知识。
7. 获取到内容后，除了专有名词（比如 Context Engineering，Vibe Coding，Harness Engineering 等）保留英文外，其他内容全部翻译为中文。

不同格式的内容，你需要不同处理。
网站：需要先获取网站内容，然后判断是视频、文章、还是播客，再做具体处理。
视频：竭尽所能拿到完整的 transcript，不需要时间戳。YouTube 视频可以使用 youtube-transcript
播客：竭尽所能拿到完整 transcript，通过搜索、写脚本、寻找视频版本等不同方式。
如果是 twitter，请你用这个方法摄取：1. 先用 r.jina.ai 抓 X 页面文本。 2. 如果只拿到登录壳
...
8. 处理原则遵循 Atomic Notes：每条笔记只表达一个明确主题或一个独立知识单元，不要把多个概念、多
...
你完全可以围绕不同场景，定义不同类型的输出 skills。例如：

1. 文章输出 skill：生成图文并茂的长文。
2. 信息图输出 skill：生成信息图片或视觉卡片。
3. PDF 输出 skill：生成适合归档或分享的 PDF 文档。
4. PPT 输出 skill：生成演示文稿结构和页面内容。
5. 研究备忘录输出 skill：生成内部 memo。
6. 社交媒体输出 skill：生成 thread、短帖、海报文案。
...
每一类输出 skill 都可以有自己的产物格式。例如：

1. `article/` 里放 Markdown 长文。
2. `infographic/` 里放图片说明稿、设计文案、生成脚本或最终 PNG。
3. `pdf/` 里放导出的正式文档。
4. `ppt/` 里放演讲提纲、逐页内容、最终演示文件。
...
3. 让 AI 回答一个具体问题，并把回答存到 `outputs/qa/`。
...
2. **Areas**：长期持续负责的领域
3. **Resources**：参考资料和主题知识
4. **Archives**：暂时不用但需要保留的内容
...
1. `fleeting/` 放临时想法
2. `literature/` 放基于资料的阅读笔记
3. `permanent/` 放经过整理后的长期知识卡片
4. `index/` 放索引和导航页
...
2. 如果你更关心主题导航和知识浏览，用 MOC。
3. 如果你更关心概念沉淀和长期写作，用 Zettelkasten。

也可以混用。比如：

1. 顶层用 PARA 管理大类。
2. 中间层用 MOC 做导航。
3. 底层用 Zettelkasten 存概念卡片。

这也是比较适合 AI 的方式，因为它同时兼顾了结构、导航和原子化沉淀。

---

## 10. 结尾：这不只是知识库，这是 Compound Engineering
...
1. 不要只让 AI 生成一个结果。
2. 要让 AI 帮你搭一个能持续生成结果的系统。
3. 这个系统要能积累、能修正、能输出、能自检。
